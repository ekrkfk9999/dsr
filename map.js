...
function createCheckbox(labelText, checkboxId, iconName, mapData, currentArray) {
    ...
    input.addEventListener('change', function() {
        ...
        currentArray.forEach(item => {
            // 만약 item이 객체라면 (몹 이미지의 경우)
            if (item.mobImage && item.typeImage) {
    ...
    mapData.forEach(item => {
        ...
        if (checkboxId === 'toggle-mob') {
            ...
            typeElement.style.width = `20px`;
            typeElement.style.height = `19px`;
            typeElement.style.zIndex = `1000`;
            typeElement.style.display = input.checked ? 'block' : 'none';

            // 몹에만 특별한 툴팁 적용
            addSpecialTooltipToMobs(imgElement, item.name, item.src, item.level, item.hp, item.강점, item.약점, item.items);

            // 이미지 컨테이너에 몹과 타입 이미지를 추가
            imageContainer.appendChild(imgElement);
            imageContainer.appendChild(typeElement);

            // currentArray에 몹과 타입 이미지를 객체로 저장
            currentArray.push({ mobImage: imgElement, typeImage: typeElement });
        } else {
            // 일반 아이콘에 툴팁 추가
...
// mobs 전용 툴팁 처리 함수
function addSpecialTooltipToMobs(imageElement, name, src, level, hp, 강점, 약점, items) {
    imageElement.addEventListener('mouseenter', function(event) {
        showSpecialTooltipAtImage(event, imageElement, name, src, level, hp, 강점, 약점, items); 
    });
    imageElement.addEventListener('mouseleave', hideSpecialTooltip); 
}
function showSpecialTooltipAtImage(event, imageElement, name, src, level, hp, 강점, 약점, items) {
    let tooltip = document.createElement('div');
    ...
    tooltip.innerHTML = `
        <div style="text-align: center; font-size: 20px; font-weight: bold; color: white;">${name}</div> <!-- 이름 중앙 정렬 -->
        ...
        <div style="text-align: center; font-size: 20px; font-weight: bold; margin-top: 10px; color: white;"><strong>드랍 아이템</strong> 
            <ul style="margin-top: 5px; list-style-type: none; padding-left: 0; font-size: 14px; text-align: left;">
                ${드랍아이템목록.map(item => `
